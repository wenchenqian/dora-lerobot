...
在megatron训练框架中，权重文件有分布式（dist-checkpointing）和非分布式两种，两种权重文件的区别如下：

* **非分布式：需要DP域内汇总各个rank的部分模型权重参数，然后由DP域内的rank0进行权重保存，但仅仅为汇总分布式优化器上划分的优化器状态域参数，还是会有`PPxTP`个子权重文件，这些组合起来才是一个完整的模型参数和优化器状态等数据。**
* **分布式：更好地与megatron的model parallel和分布式优化器配合，分布式优化器里的切分分布到所有DP域内的优化器状态也不需要汇总到DP域0的所有buffer内，而是直接保存，这样就会有`DPxPPxTP`个权重文件。**

下面直接上图：![image.png](assets/image.png)
