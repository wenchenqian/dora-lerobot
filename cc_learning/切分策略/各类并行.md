...
![image.png](assets/image3421432.png)


![image.png](assets/image45323.png)

![image.png](assets/image56654365.png)

![image.png](assets/image43254325.png)
...
流水线并行主要解决模型层间的并行问题，对于长序列输入（如文本或视频）的处理仍存在瓶颈。这时，长序列并行登上了舞台。长序列并行是一种专注于序列数据的并行策略，它将输入序列（如文本中的token序列）分割成多个片段，分配到不同设备上并行处理。

长序列并行的基本目标是解决注意力矩阵的激活值内存随序列长度平方增长的问题，并延伸出多条不同路线的长序列并行策略
...
Flash Attention并不是严格意义上的长序列并行，但是它对注意力矩阵的计算有着极大的开创性，为后续长序列并行的提出有着重要的启发，因此我们还是将它作为长序列并行的第一个方法进行介绍。

Flash Attention 的核心思想是：避免实例化完整的 N x N 注意力矩阵，通过将计算分块（Tiling）和重计算（Recomputation）技术，在 GPU 的高速 SRAM（即共享内存/缓存）中进行整个注意力计算的全过程，只将最终的、尺寸较小的结果写回 HBM。这就像要计算一个巨大的 Excel 表格的总和，传统方法是把整个表格加载到内存里再算，而 Flash Attention 是每次只加载一小块（一个 Tile）到最快的缓存里进行部分计算，并不断更新一个中间结果，最后得到总和，从而避免了加载整个大表格。

Flash Attention的关键技术可以概况为一下三点：

1.1 Tiling：将输入Q,K,V分块，将它们从慢速的HBM加载到快速的SRAM中，然后计算关于这些块的注意力输出。通过在将它们相加之前用归一化因子缩放每个块的输出，得到最终的结果；

1.2 重计算：在正向传播过程中不存储注意力矩阵，在反向传播时通过之前存储的softmax函数归一化因子快速的对其进行重计算；

1.3 内核融合：从HBM加载输入后，执行所有计算步骤（矩阵乘法，softmax，可选的mask和dropout ）后再写回HBM，避免反复读写HBM的输入和输出；
![img_11.png](img_11.png)
![img_13.png](img_13.png)
2. ULYSSES

ULYSSES 的核心思想是：将输入序列本身在设备间进行划分，并通过高效的通信重组，使得每个设备只需要处理全局序列的一个子片段，同时又能计算出正确的全局自注意力。
![img_12.png](img_12.png)
ULYSSES可以显著降低激活值内存，允许序列长度随着GPU数量线性增长，并且可以与DP，TP，PP等并行策略轻松结合，同时可以与Flash Attention配合使用，从而在超大规模的集群上训练极大数据的模型。

但ULYSSES同样存在问题。主要问题来源是ULYSSES需要All-to-All通信，通信量与序列长度和隐藏层维度成正比，且通信时间遮掩更加复杂和困难。同时ULYSSES的并行度必须能够整除注意力头数，这限制了它的可扩展性。
3. Ring Attention

Ring Attention是一种将模型并行与环状通信相结合的高级并行范式。它的核心思想是：将超长序列分割成多个较短的子序列块，将这些块分布在一个设备环（GPU/NPU环）上，通过类似“传送带”的方式，让每个设备都能逐步与整个序列的所有块进行计算，最终计算出完整的全局注意力。
想象一个环形工厂流水线：

分块：一个很长的产品（长序列）被切成多个小部件（子序列块）。
分配工位：每个工位（GPU/NPU）负责处理一个部件。
传送带：工位之间有一个环形的传送带。每个工位在加工自己的部件时，会把一个“模板”或“信息”（即 KV Cache）放到传送带上。
逐步组装：传送带缓缓移动，每个工位不仅能加工自己的部件，还能陆续收到来自其他所有工位的“信息”，利用这些全局信息来完善自己的加工过程。
最终产出：经过一圈的传递，每个工位都接触到了所有部件的信息，产出了完美的最终产品（每个子序列块的输出都融入了整个长序列的上下文信息）。
...
![image.png](assets/image4e3254325.png)
