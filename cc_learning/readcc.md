...
若是 GroupGemmExperts LoraGroupGemmExperts 适配 MoE 专家层。在 MoE 中，每个 token 被路由到少数专家（如 2/8），形成稀疏激活。为高效计算，使用GEMM： 所有 token 按专家分组 对每组独立做矩阵乘（专家权重 × 该组 token） 最后 unpermute 回原序。如果启用了 专家并行（Expert Parallelism），则 LoRA 参数 不参与全局 AllReduce（因为每个设备的专家是独立的）。
...
“Moonlight版”，它的缩放因子是通过对齐Adam的Update RMS得到的，从实验中观察到，大致上在Warmup结束、模型进入正式训练后，Adam的Update RMS几乎都保持在0.2～0.3之间，并且不同尺寸的模型也呈现出相似的规律。这些模型的共同点是都用Adam训练，参数是β1=0.9,β2=0.95
...
工行里面有一套【风险分析框架】，包含信用健康度、经营稳定性和债务流动性，用了贷记卡额度使用率、金融资产余额、存款余额等信号，目前的开源数据中很难找到完全吻合，但有类似信号可以用

结合Deepseek和人工分析，建立一套适用于开源数据的【风险分析框架】，包含短期流动性、长期偿债能力、盈利能力、资产效率和现金流健康度，从原始20+特征里提取17个特征作为【企业特征数据报告】
...
thinkingmachinelab，在线蒸馏策略（得到密度反馈），在特定领域达到专家级别。强化学习计算成本高。损失函数：反向KL散度，让学生在自己的上下文中尽量预测出教师认可的token。正向KL可能会钻空子，反向是从学生角度出发
...
DeepSeek蒸馏，通过蒸馏技术，快速使得快思考模型具有慢思考能力。将蒸馏技术应用于盘古大模型的增强过程，通过一系列实验验证蒸馏技术增强盘古模型的有效性。
...
* 中文开源DeepSeek-R1 蒸馏推理数据集（110K 样本）：数据分布如下Math：共计36568个样本，Exam：共计2432个样本，STEM：共计12648个样本，General：共计58352，包含弱智吧、逻辑推理、小红书、知乎、Chat等。
...
DeepSeek-R1-Distill-Qwen-32B 通过80万条蒸馏数据，经过SFT微调得到；而DeepSeek-R1-Zero-Qwen-32B则采用R1-Zero训练流程，并进行了10k以上的大规模RL训练。这一结果表明，在冷启动阶段，知识蒸馏是提升模型性能的重要手段。R1训练过程中使用了多种数据类型，特别是在冷启动阶段，引入了高质量的CoT数据以及RL生成的数据，并基于Base模型进行训练。这些处理有效解决了R1-Zero早期优化过程中因随机策略带来的不稳定性问题，同时减少了 RL训练过程中常见的语言混合和可读性问题。
