**TP-SP**  在（TP） 基础上，****将序列维度****；每个 GPU 只持有序列的一部分，****共同完成同一个 attention head 的计算****。
...
Token 路由需要所有参与 EP 的设备在同一 PP stage 内通信（如 AllToAll）如果 EP 跨越多个 PP stage，则路由无法完成（因为 activation 还没传到下一个 stage）
高 TP + 高 PP // TP 的 AllReduce + PP 的 P2P 通信竞争带宽
高 EP + 高 TP // GMM 计算 + NPU AllToAll 路由成为瓶颈

rope绝对位置编码，qk向量乘旋转矩阵rm。高频：短距离依赖。低频：长距离依赖.rope里每个m对应弧度m*theta—i

解决外推可以用NTK，保留高频信息，高频部分（旋转周期数量足够多）进行外推，低频部分进行内插

增量预训练完成后通常还需要SFT重新对齐为可对话的chat模型，此时进行评测后发现模型效果劣化严重可能已经浪费了许多宝贵的时间以及算力。megatron原生的loss打印机制为打印一个batch所有token的平均loss可以反映数据的难易程度，如果某些channel的初始loss较高，且一直不下降，可能是这部分数据比较脏或者比较难，需要进一步检查数据质量
...
参考后训练论文调整不同领域的训练数据配比 动机：为什么要在训练过程中动态的reweight？
...
LoRA 的核心只是把参数更新限制在一个低秩子空间上，引入低秩矩阵来近似权重的增量更新，（通过 SVD 分析发现：微调后得到的 ΔW，其前几个奇异值远大于其余值，说明 ΔW 的能量集中在少数主成分上 —— 即近似低秩。）但损失函数依然是普通的cross-entropy，所以在小数据 / 大步长的设置下，依旧可能严重偏离原始分布；同时，rank 也很难选：太小学不动复杂模式，太大又增加参数与显存开销、训练不稳、更容易过拟合。
...
若是 GroupGemmExperts LoraGroupGemmExperts 适配 MoE 专家层。在 MoE 中，每个 token 被路由到少数专家（如 2/8）。为高效计算，使用GEMM： 所有 token 按专家分组 对每组独立做矩阵乘（专家权重 × 该组 token） 最后 unpermute 回原序。如果启用了 专家并行（Expert Parallelism），则 LoRA 参数 不参与全局 AllReduce（因为每个设备的专家是独立的）。
// 将 所有本地专家 的权重 拼接成一个大矩阵：
lora_A: 形状 = (num_exp * d_in, r)
lora_B: 形状 = (num_exp * r, d_out * 2) （*2 因 GLU 激活） 为了高效实现 SwiGLU，（ Megatron）会将 W_up 和 W_gate 的权重拼接在一起，形成一个大的权重矩阵：
...
再 npu_gmm 计算 base + LoRA3、unpermute4、处理share expert
...
使用“Moonlight版”，它的缩放因子是通过对齐Adam的Update RMS得到的，从实验中观察到，大致上在Warmup结束、模型进入正式训练后，Adam的Update RMS几乎都保持在0.2～0.3之间，并且不同尺寸的模型也呈现出相似的规律。这些模型的共同点是都用Adam训练，参数是β1=0.9,β2=0.95

Muon并不是只管矩阵参数，Muon只管“稠密输入的线性层的矩阵参数”,Embedding层和最后的分类层（包括GPT的LM Head）的矩阵参数都不能用Muon，否则效果会明显差。这些不能用Muon的矩阵参数，直接用Adam就行，基本上Muon实现都是混合了Adam的，用户可选某些层用Adam。
...
RMS系数:梯度平方的指数移动平均（EMA）的平方根，是一个自适应的、基于历史梯度尺度的归一化因子优化器中，通常使用指数移动平均（EMA）代替简单平均，以更关注近期梯度，缓解梯度消失/爆炸![img_4.png](img_4.png) 作用是![img_5.png](img_5.png) ADAM
...
对齐RMS的系数对SFT的影响很大。如何快速找到接近最优的超参数，比如Muon至少有学习率ηt和衰减率λ两个超参数。提出Update RMS对齐的超参迁移思路，可以将Adam调好的超参数用到其他优化器上。RMS度量了矩阵每个元素的平均大小。观察到Adam更新量的RMS是比较稳定的，在0.2～0.4之间。建议通过RMS Norm将新优化器的Update RMS对齐到0.2，这样就可以复用Adam的ηt
和λ，以达到每步对参数的更新幅度大致相同的效果。
将对齐rms的系数改为1.0，动量系数改为0.9, NS迭代改为 qutinc

### MOE
引入门控网络，由多个Experts组成，每个专家是一个独立的子网络，每次前向只使用部分专家topk
...
流水线并行PP：扩大PP可以缓解单卡内存压力，但是其过大易引入空泡，且降低了通信计算掩盖程度；PP减少会导致内存压力增加，被迫大量使用重计算后会导致吞吐下降。因此，在内存使用允许的条件下尽量减少PP，最终确定为PP16。为了优化PP时的通信性能，同时考虑内存节省，不同于DualPipe（双流缓存），PanguUltraMoE718B使用VPP降低空泡，可以减少
一半权重内存开销。

TP：因为专家维度为2048，对专家进行TP切分会导致 矩阵计算效率下降、TP通信需求过高的问题，因此，并行策略中TP只作用于Attention区域，不做MLP部分的切分。考虑采用昇腾800TA2集群，单机内8卡组成Mesh高速带宽域，
使用TP8可以最大限度的利用该高带宽进行TP通信，且有效的降低Attn部分的权重和激活值占用。

专家并行EP：EP、TP解耦特性，EP通信域往往同时包含两级。以EP32为例，其通信域构成为4组8卡机器，其中跨机器通信效率较低。为最大限度
...
家数增加，内存压力上升，同时EP小会让整个EP域内供分配的Token数量降低，影响负载均衡。在反向梯度累加计算中，EP减少将导致单卡专家数增多，则add算子的计算量上升，导致整体计算效率下降。因此，在昇腾集群特性和分级通信算法的影响下，综合通信与计算两方面效率的整体考量，EP为32时模型训练可达到最优性能。
...
在预训练最后的阶段提高数学、代码数据可以有效提升模型在关键项上的表现。
一般包含两个部分：
...
注： 文本相似度算法建议采用ROUGE。 阈值设置没有固定值，需要根据实际情况决定，一般在0.2左右。

数据管理：基于embedding（gte-large-zh）和 FAISS 向量索引的 SFT 数据去重与查重系统：金融 QA 对语义敏感，阈值不宜过低： 0.95~0.98：严格去重（推荐）

self-instruct（提升问题多样性）:单轮对话数据合成流程：1、选择种子指令&确认指令生成数量2、拼入指令生成提示词模板： 相似指令生成：生成的指令与提供种子指令的意图、话题、问题类型、风格、难度等更加相似。 指令泛化：基于提供的种子指令进行泛化，生成的指令在保证话题相同的基础上会更加多样化。3、将生成的指令x拼入答案生成提示词模板
...
2、偏好引导： 如果明确评测标准，可以将客户评判标准写入 System Prompt 来提升模型回复与评测标准一致性。以面试助手为例，评判标准有以下几点：那么可以将这些要求在数据合成过程中输入给回复生成模型，让回复更贴近评判标准。
...
3、拒绝采样，同时利用反馈信息，整体提升对齐效果。有些任务， 即使经过了偏好对齐采样， 依然不能得到有效提升。 主要有两个原因: 1. 即使使用偏好对齐采样，教师模型的答案依然不够好。 2. SFT没有利用负样本，单纯依靠正样本训练，不能让模型充分了解什么样的答案是不合适的。 可以考虑增加反馈信息。
...
难度筛选方式：一般应用于有标准答案的数据进行难度筛选，通过调用较差模型获取多答案，再经过答案一致性判定答案是正确答案的比例，如果比例超过60%，认定为简单题目，反之为复杂题目。
...
- 工行数据：4w（可选）
- 人设/安全/电诈/信用：~0.3w
  强调慢思考——即需要多步推理、专业知识、严谨表达的任务； 包含安全合规子集，满足金融监管要求；
...
重点能力项数据构建：工单智能分发：主要问题： 格式不遵从，会添加额外的说明部分。1、从评测集中梳理出针对表单问题的不同提问方式，和标准输出。梳理出了全部的9种

摘要：客户反映什么时间遇到什么问题，坐席引导/安抚/解释，客户是否认可，是否提出新的要求/投诉。
网点信息：
身份证号：
卡号：
联系方式：
...
根据以上背景内容，将信息填入模板中。
1.只需要返回填写后的完整模板信息。请返回完整模板，不要返回任何其他内容。
2.如果模版中的需要提取的信息在对话里没有提及，则填入“【】”即可。
3.如果模板为空，则直接输出背景，不要返回任何其他内容。
4.你不能修改模板中除空白以外的其他任何内容。
"""

moban_1_output_sample = """
摘要：客户反映之前被告知办理扫码不收费，但最近发现开始收费。坐席引导客户进行投诉和登记反馈，客户不满意要求处理，决定投诉。
网点信息：陕西省渭南市前进路支行
身份证号：【】
卡号：【】
联系方式：80622866567

2、利用DeepSeekR1进行答案获取：工单 + "\n" + moban + "\n请严格按照输出样例的格式进行输出，不要发散：\n" + output_sample

3、数据质检  deepseek v3  4、质检不通过的答案进行改写  DeepSeek V3
...
注： A的作用是生成问题，B的作用是生成答案，因此A和B本质上可以是一个模型，通过不同的调用方式或提示词进行区分即可。若考虑到效果和效率，则也可以采用不同的模型。生成问题的任务比较简单，因此A可以是一个参数规模较小的模型，注重效率；生成答案的任务更加考验模型能力（或者需要同时生成答案和思考过程），B建议是一个能力比较强（或者具有reasoning能力的模型）的模型，注重质量

### 金融项目

分为金融基础能力【金融计算、金融摘要生成等】和金融应用场景【企业风险评估、智能工单生成】

工行里面有一套【风险分析框架】，包含信用健康度、经营稳定性和债务流动性，用了贷记卡额度使用率、金融资产余额、存款余额等信号，目前的开源数据中很难找到完全吻合，但有类似信号可以用。结合Deepseek和人工分析，建立一套适用于开源数据的【风险分析框架】，包含短期流动性、长期偿债能力、盈利能力、资产效率和现金流健康度，从原始20+特征里提取17个特征作为【企业特征数据报告】

在【强制约束】里要求综合考虑正负向信号，同时输出要求里只有三个评级结果，A/B/C（AA, AAA都算作A），统计AA的区分度

工行企业风险智能评估例子：【角色】【任务指令】
...
trpo-ppo-grpo trpo：ratio（新旧策略差别有多大）*advantage， 用KL散度进行计算（相当于加入一个限速器，限制ration）
...
ppo代表onpolicy（loss基于ratio，用clipping限制两个分布），DPO（降低不好回答被采样的概率，提升好回答的概率）代表offpolicy，DPO将RL问题转化为一个基于偏好数据的分类问题，绕过了奖励建模和Critic。

强化学习合成数据：1、知识引导合成：首先会使用大模型从任务描述中提取一组领域特定的关键词。这些关键词精确地概括了任务的核心领域与要求。接下来，一个「段落检索器」会使用这些关键词，在一个大型的高质量文本库中进行搜索，从而找到一系列与任务高度相关的知识段落。获取相关的知识段落后，生成器开始合成初始的任务样本集。LLM生成器会综合利用包括之前检索到的相关段落、抽象模式与具体示例的组合，以及多个原始的任务指令，来生成初始合成数据集。并通过majority vote确保任务输出的正确性。通过这种方式确保了合成出来的数据不仅在事实上有所依据，而且在形式和内容上也更加丰富。2、难度自适应（解决训练样本难度不均衡的问题，过于简单的样本可能无法提供足够的改进奖励，而过于复杂的样本则可能阻碍模型的有效学习能力。通过自动评估和改写样本，生成一个针对预训练模型难度分布更合理的数据集，从而提升最终效果），系统会根据模型的反馈来调整这些生成样本的复杂度，目的是创建一个难度均衡、避免过于简单或困难的数据集。（1）全面评估，样本分为已解决和未解决（2）利用一个llm改写器对已分类的样本进行难度调整（3）将三个部分的数据合并在一起，包括原始的初始样本集、新生成的更难样本集、新生成的更容易样本集。

通过难度自适应策略生成的包含多样化难度的大规模合成数据集，并不会直接用于训练，因为许多合成样本可能对预训练模型来说过于简单或过于困难，无法提供有效的学习信号。

为了最大化训练效率和效果，设计了第三个环节，为了识别并利用那些最具学习价值的「高潜力」样本。为了精准地识别高潜力样本，设计一套基于模型实际表现的评分系统。
利用基础模型，对每个样本进行多次解答尝试。接着，计算模型在多次尝试中成功解答的次数比例。设计：对于那些模型在所有尝试中都失败的「极难」样本（即通过率为0），故意给它们一个最高分（比如1）。这样做的目的是为了在后续排序时，能够轻易地将这些过于困难/存在合成错误的样本沉底。评分完成后，所有样本会按照它们的「通过率得分」从低到高进行排序。根据这个排序结果，得分最低（但大于0）的样本，正是我们寻找的「高潜力」目标—模型偶尔能答对，充满了不确定性。框架会从排序列表的顶端选取一定数量的样本，构成训练集。选出的高潜力训练集将被用于对基础模型进行一轮的强化学习训练。这个步骤旨在将模型在这些「临界区」样本上的不确定性转化为稳定的正确解答能力
...
通过训练过程中多观察一下输出结果，发现 RM **漏判的 bad case**（如胡说但高分），发现 RM **误杀的 good case**（如正确但低分），识别未覆盖的场景（如金融术语）。持续refine奖励函数

主要是选择题的rewardfunc，引导模型学习规范输出格式。强制要求答案必须用 `\boxed{...}` 格式；如果格式不对 → 返回 `-1`（惩罚）；如果格式对但答案错 → 返回 `-0.5`；如果格式对且答案对 → 返回 `1`。

1、多选题格式多样（A,B / A和B / AB）→ 漏算“多选正确”。优化：`parse_answer` 支持多种聚合形式。未优化漏算案例：

* 模型输出：`"AB"`
* 标准答案：`"A,B"`
* naive reward：`"AB" != "A,B"` → 返回 0（漏算
* 更糟的是：

  * 模型输出：`"选项 A 和 C 正确"`
  * naive reward：完全不匹配 → 0 分。
* 我的版本：识别为 `{'A','C'}` → 正确评分。

  解析策略（三层保险）：

  1. **聚合模式**：如果字符串只包含字母和逗号（如 `"AB,C"`），按逗号拆分后展开字母；
...
reward 只有一个，怎么分配到每个 token？ PPO：通过 GAE 或 reward shaping 分配 advantage 到每个时间步。grpo：不把 reward 分配到每个 token，而是对整条序列计算一个 scalar reward，然后将该 reward 广播到序列中所有token的loss计算中。
...
理想情况：总数据分片数能被 DP 整除，例如 1024 个 tar 包 + DP=512 → 每个 rank 读 2 个。
问题情况：存在大量 不能被 DP 整除的分片数量，例如 1000 个 tar 包 + DP=512。rank（尤其是 DP=0，即 TP0 所在的 DP rank）需要跨多个tar包读取极少量数据。
为了读取几个样本，必须打开并解压多个 tar 文件，造成 I/O 放大（I/O amplification）。
...
multiprocessing或类似并行框架在处理多个 .json 文件时，子进程完成顺序是不确定的。最终拼接的 token 序列顺序可能不同。
...
1、明确动机（1）研究动机，明确benchmark，开源数据2、领域特殊性，安全合规

想一想模型类型，用稠密，moe还是混合，模型大小，数据混合方式，确定训练什么，所有决策通过消融实验决定

对比是否绑定嵌入

后训练：循序渐进，先sft，再dpo（偏好优化），最后实时反馈优化grpo+长度惩罚，需要仔细设计奖励函数

## 最近看到的

小米采用混合注意力，具体是 1:5 的全局注意力与滑动窗口注意力混合策略。这里之所以选择混合的 ,是因为小米团队通过实验发现:SWA 简单高效且易于使用,在通用任务、长上下文处理和推理能力上,其整体表现优于线性注意力

采用了一种新的后训练策略:多教师在线策略蒸馏其核心是高效的在线策略学习机制:在通过 SFT/RL 获得领域专家教师模型后,学生模型从自身策略分布中采样(rollout),并利用多个教师提供的密集、逐token奖励进行优化。

off-policy训练的缺点在于,学生模型学习的上下文是教师常出现的情境,而不是学生自身经常会遇到的情境。这可能导致累积误差:如果学生在早期犯了个教师从未犯过的错误,它会逐渐偏离 训练中观察到的状态。当我们关注学生在长序列上的表现时,这一问题尤为突出。为了避免这种偏离,学生必须学会从自身的错误中恢复。off-policy distillation的另一个问题是,学生可能学会模仿教师的风格和自信,但不一定掌握其事实准确性。

attention的优化，gemma3使用滑动注意力窗口，qwen3使用DCA分块，但是next使用线性注意力

thinkingmachinelab，在线蒸馏，在特定领域达到专家级别。强化学习计算成本高。损失函数：反向KL散度，让学生在自己的上下文中尽量预测出教师认可的token。正向KL可能会钻空子，反向是从学生角度出发

1、采样学生轨迹，让学生模型生成自己的轨迹，记录每个token的对数概率2、计算奖励，学生轨迹传给教师客户端，得到教师对每个token的logprobs，用学生的logprobs减去教师logprobs得到反向KL散度，散度越小，奖励越高3、强化学习训练，每个轨迹的advantages设置为反向KL散度取负，调用importance sampleing更新学生模型参数

可以扩展到更复杂的持续学习场景，每个月给模型更新一次行业数据，然后做在线策略蒸馏。密集监督是提升计算效率的关键，密集过程监督比结果监督更有效

强化学习的核心是搜索，通过大量试错，寻找解决问题的语义策略，online相当于跳过搜索过程。可以用强化先找到好的策略，然后小模型蒸馏，快速落地

## 快慢思考融合训练

通过这种方式，能够在保持快思考模型原有优势的同时，通过新设定的慢思考人设在高效果要求的场景中提供了更深层次的推理和更高的准确性。原学生模型为Chat模型，即快思考模型，因此快思考人设为学生模型的默认人设。相比于快思考人设，慢思考人设在其基础上进一步融入了推理的需求，强调了推理思考的过程：

* **快思考人设**：你是一个擅长于文章写作、文本分析、代码编写、数学运算、逻辑推理的AI助手，专注于针对用户的问题给出高质量解答。
* 慢思考人设：你是一个擅长于文章写作、文本分析、代码编写、数学运算、逻辑推理的AI助手，专注于针对用户的问题给出高质量解答。你的解答中包含推理思考与最终响应两部分，请先在<think>...</think>标签中进行推理思考，再给出最终响应。

DeepSeek蒸馏，快速使得快思考模型具有慢思考能力。通过一系列实验验证蒸馏技术增强盘古模型的有效性。

主要使用诺亚所蒸馏的高质量慢思考通用数据以及补充了部分中文开源数据。

诺亚慢思考数据的准备：选择 Query候选数据集和回复的过程中，主要围绕数学领域从知识广度、难度层级和中英文配比出发:

* **知识广度：**数学领域也涉及多个子学科。筛选问题时，确保涵盖不同数学分支，并涉及多元化的概念体系，以迫使模型在解题时建立跨学科连接，提升其推理能力。
* **难度层级：**重点筛选具有挑战性的复杂问题，构建多层次的推理链条，以增强模型的长链推理能力。这些问题主要来源于各类数学竞赛的历史题。
* **中英文配比：**为了训练具备双语长链推理能力的模型，我们确保训练数据的中英比例为 2:1。
* **回复质量提升：**通过多轮拒绝采样，构建高质量的长CoT回复。**
* 中文开源DeepSeek-R1 蒸馏推理数据集：数据分布如下Math：共计36568个样本，Exam：共计2432个样本，STEM：共计12648个样本，General：共计58352，包含弱智吧、逻辑推理、小红书、知乎、Chat等。
...
2. 构造混合数据集：
3. **用 GRPO/PPO 进行 RL 微调**，设计 reward
   * **快思考 prompt → 奖励简洁 + 正确；**
   * **慢思考 prompt → 奖励详细推理 + 正确；**

慢思考数据合成：由于行业数据为快思考数据，需要通过教师模型将其重新合成为慢思考数据。 数据生成：通过DeepSeek-R1大模型，对所有行业数据的问题，进行带有思考答案的重新生成。数据清洗：通过Deepseek-V3大模型，结合人工对R1的结果进行verify，保留高质量回复。

DeepSeek-R1-Distill-Qwen-32B 通过80万条蒸馏数据，经过SFT微调得到；而DeepSeek-R1-Zero-Qwen-32B则采用R1-Zero训练流程，并进行了10k以上的大规模RL训练。这一结果表明，在冷启动阶段，知识蒸馏是提升模型性能的重要手段。R1训练过程中使用了多种数据类型，特别是在冷启动阶段，引入了高质量的CoT数据以及RL生成的数据，并基于Base模型进行训练。这些处理有效解决了R1-Zero早期优化过程中因随机策略带来的不稳定性问题，同时减少了RL训练过程中常见的语言混合和可读性问题。

* **训练阶段**：
  1. 利用多种方法生成冷启动长CoT数据，包括提示工程合成和DeepSeek-R1-Zero生成，并经过人工校验得到高质量的长CoT数据，微调DeepSeek-V3-Base模型，并经过大规模强化学习训练提升模型的推理能力。
  2. 在冷启动的SFT和RL训练后，生成的中间模型并非最终的R1模型，而是一个高质量长CoT数据合成模型，然后使用拒绝采样方法合成具有更高质量推理轨迹的长CoT数据。对于慢思考模型至关重要。
* **蒸馏阶段**：基于80万条长CoT数据，DeepSeek对不同参数量级的开源模型进行了微调。

**RL训练基座的影响：Pretrain vs. Instruct**

强化学习的初始策略模型是影响训练效果的关键因素之一，对比了 Qwen-Math-7B-Base 模型和 Qwen-Math-7B-Instruct 模型对 RL 训练所产生的影响。基于快思考 Instruct 模型进行 RL 训练时，往往在较短的训练步数（例如 40 步）就出现了过早收敛甚至过拟合的迹象。在后续继续训练至 100 步或更多步数时，并没有带来明显的精度提升，反而可能会出现推理长度变短、泛化能力下降等问题。相比之下，基于预训练 Base 模型进行相同的 RL 训练时，尽管在初始阶段的表现可能弱于Instruct模型，但在较长的训练步数范围内仍能够保持稳步提升，并未出现与 Instruct 模型同等程度的过早收敛问题。

* 训练早期（<40步）：在测试集上的表现提升较为显著，准确率从 43% 提升至 47% 左右，同时奖励值也呈现出较为明显的增长趋势，整体表现优于同时期的 Base 模型；
...
多轮rl直接学策略，reflection死循环问题（缺少细粒度可计算信号和明确的停止条件）可以加强制措施（检测重复）

UFO：tryagain最简单的一元反馈。 结构化反思：1、基于前一步证据诊断错误2、提出正确可执行的后续调用。结合dapo和gspo，优化反思，调用，完成的分步策略

ToolRL四个维度的奖励配置：1、奖励类型2、奖励尺度3、奖励粒度（细粒度奖励分解更稳定学习）4、奖励动态（动态奖励尺度有助于模型从简单过渡到复杂）
