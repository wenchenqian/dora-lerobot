...
#### 3. **可选：打印调试信息（仅 rank 0）**
```python
if args.return_document_ids and ... == 0:
    print("current idx: {}, ..., document_ids: {}".format(...))
    batch.pop('document_ids', None)
    batch.pop('idx', None)
```

- **目的**：调试数据来源，确认是否按预期混合了不同数据集；
- **仅在全局 rank 0 打印**：避免日志爆炸；
- **立即 pop 掉**：这些字段不参与训练，防止内存浪费或传输开销。
...
✅ 这是**非常精细的工程处理**，确保 metadata 不被错误切分。
...
结合你之前的上下文：
- 你在 **NPU 上训练**，使用 **Megatron-LM 风格框架**；
- 混合了 **通用数据、chat 数据、领域数据**（通过 `dataset_id` 区分）；
- 使用 **上下文并行**（CP） 处理长序列；
- 关心 **增量预训练 vs SFT 的 loss 计算**。
...
1. **`dataset_id` 处理可更优雅**  
   可在 `get_batch_on_this_cp_rank` 内部增加 `metadata_keys` 参数，自动跳过切分。

2. **`actual_seq_len` 可能冗余**  
   若 CP 切片已知，序列长度可直接计算，无需额外返回。

3. **调试打印可加采样率**  
   避免每 step 打印，可用 `if step % 100 == 0`。
